...
export async function getDb() {
    ...
    initializationPromise = (async () => {
        ...
        try {
            ...
            // Check if seeding is needed (e.g., if content is empty)
            const contentCount = await db.get('SELECT COUNT(*) as count FROM content');
            if (contentCount.count === 0) {
                ...
                await db.run('INSERT OR IGNORE INTO users (name, email, password, role) VALUES (?, ?, ?, ?)',
                    ['Admin', 'admin@royalcaredental.co.ke', 'admin123', 'admin']);
            }
