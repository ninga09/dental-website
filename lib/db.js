...
const db = {
    ...
    exec: async (sql) => {
        try {
            ...
            // During build phase, we might not have a writable DB.
            // Next.js static generation might try to run this.
...
export async function getDb() {
    // Skip initialization if we're in the build phase and it fails
    if (!initialized) {
        try {
            await db.exec(`
